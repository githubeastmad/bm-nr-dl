...
def searchsimilar(inputsearchfeature):
    #input_feature_origin,input_feature_denoised, search_feature_origin, search_feature_denosied,N1
    #
    #
    #
    #
    #
    input_feature_origin = inputsearchfeature[0]
    input_feature_denoised = inputsearchfeature[1]
    search_feature_origin = inputsearchfeature[2]
    search_feature_denoised = inputsearchfeature[3]
    k = inputsearchfeature[4]


    input_feature_origin = np.exp(input_feature_origin)
    input_feature_denoised = np.exp(input_feature_denoised)
    search_feature_origin = np.exp(search_feature_origin)
    search_feature_denoised = np.exp(search_feature_denoised)
    ...
    for view_input in range(input_feature_origin.shape[0]):
        for view_search in range(search_feature_denoised.shape[0]):
            for i in range(search_feature_origin.shape[2]):
                for j in range(search_feature_denoised.shape[3]):

                    compare_feature_origin = search_feature_origin[view_search, :, i, j]
                    compare_feature_denoised = search_feature_denoised[view_search, :, i, j]
                    input_feature_single_origin = input_feature_origin[view_input, :]
                    input_feature_single_denoised = input_feature_denoised[view_input,:]

                    input_feature_single_denoised = np.sqrt(input_feature_single_denoised)
                    compare_feature_denoised = np.sqrt(compare_feature_denoised)

                    # input('Ready for UUU')
                    if np.array_equal(input_feature_single_origin, compare_feature_origin):
                        continue
                    dist = np.sum(np.log(np.sqrt(np.divide(input_feature_single_origin, compare_feature_origin)) + np.sqrt(
                        np.divide(compare_feature_origin, input_feature_single_origin)))+2.0*np.square(input_feature_single_denoised - compare_feature_denoised)/np.multiply(input_feature_single_denoised,compare_feature_denoised))

                    if mindist[k - 1][0] > dist:
                        mindist[k - 1] = (dist, np.log(compare_feature_origin), np.log(input_feature_single_origin))
                        # minfeature[k-1] =
...
if __name__ == '__main__':
    ...
    brightness_t1_1 = readtiff('./output/test/terrasar_multi_step1_02_modified2_time1.tif')
    brightness_t2_1 = readtiff('./output/test/terrasar_multi_step1_02_modified2_time2.tif')
    brightness_t1_1[brightness_t1_1 == 0] = np.min(brightness_t1_1[np.nonzero(brightness_t1_1)])
    brightness_t2_1[brightness_t2_1 == 0] = np.min(brightness_t2_1[np.nonzero(brightness_t2_1)])

    brightness_t1_1 = arr_log_normalization(brightness_t1_1)*2
    brightness_t2_1 = arr_log_normalization(brightness_t2_1)*2

    #brightness_t3_1 = arr_log_normalization(readtiff('../output/sen/multi_3_097_kernal13.tif'))
    brightness_denoised_arrays = [brightness_t1_1, brightness_t2_1]
    brightness_denoised = np.stack(brightness_denoised_arrays, axis=0)
    ...
    feature_t2_1 = point_feature(brightness_t2_1, KERNALSIZE).astype('float32')
    #feature_t3_1 = point_feature(brightness_t3_1, KERNALSIZE).astype('float32')



    feature_origin = np.stack([feature_t1, feature_t2], axis=0)
    feature_denoised = np.stack([feature_t1_1, feature_t2_1], axis=0)




    train_batch = np.empty((PAIRNUM*N1,1,KERNALSIZE,KERNALSIZE))
    target_batch = np.empty((PAIRNUM*N1,1,KERNALSIZE,KERNALSIZE))
    del_index = []
    ...
    for i in range(PAIRNUM):
        ...
        input_img_origin = brightness[:, x - KERNALSIZE:x, y - KERNALSIZE:y]
        input_img_denoised = brightness_denoised[:, x - KERNALSIZE:x, y - KERNALSIZE:y]
        input_feature_origin = input_img_origin.reshape(2, KERNALSIZE * KERNALSIZE)
        input_feature_denoised = input_img_denoised.reshape(2, KERNALSIZE * KERNALSIZE)

        search_feature_origin = feature_origin[:, :, x - SEARCHRADIUS:x + SEARCHRADIUS, y - SEARCHRADIUS:y + SEARCHRADIUS]
        search_feature_denoised = feature_denoised[:, :, x - SEARCHRADIUS:x + SEARCHRADIUS, y - SEARCHRADIUS:y + SEARCHRADIUS]




        input_search_batch[i] = (input_feature_origin,input_feature_denoised, search_feature_origin, search_feature_denoised,N1)
    ...
    for i, similarpatch in enumerate(similarpatchs):
        ...
        for j, similar in enumerate(similarpatch):
            ...
            if similar[0] / input_feature_origin.shape[1] < THRESHOLD:
    ...
    for epoch in range(7):
        ...
        for xb,yb in train_dl:
            ...
            loss = loss1 + loss2 + 0.5 * dist
            loss.backward()
            opt.step()
            #print(loss.item())


    PATH = './models/test/terrasar_multi_step2_02_05_modified2.pth'
